...
发送部分定义及变量
----------------------------------------------------------*/
static u8 xdata  SEND_BUF[UART4_SEND_BUF_SIZE]; //数据池
...
接收部分定义及变量
----------------------------------------------------------*/
static u8 xdata  Receive_BUF[UART4_RECEIVE_BUF_SIZE];  //数据池
...
/**
  ...
  * @返回值 无
  */
void UART4_Init(u32 btl)
{
	P0_MODE_IO_PU(GPIO_Pin_2|GPIO_Pin_3);   //RXD TXD 设置为准双向口
    P0_PULL_UP_ENABLE(GPIO_Pin_3);          //TXD 4.1K上拉
    UART4_SW(UART4_SW_P02_P03);             //切换引脚
...
{    
    ...
    if((end+Size)<UART4_SEND_BUF_SIZE){      //复制数据
        memcpy(&SEND_BUF[end],buf,Size);
        end+=Size;
    }
    else{
        u16 len = UART4_SEND_BUF_SIZE-end;
    ...
    if(SEND_ENDBZ){
        SEND_ENDBZ=0;
        SEND_DMA_SIZE = (SEND_APP_WR_ADDR>SEND_POINT)? SEND_APP_WR_ADDR-SEND_POINT : UART4_SEND_BUF_SIZE-SEND_POINT;
        DMA_UR4T_AMTH = (SEND_DMA_SIZE-1)>>8;               //字节数
        DMA_UR4T_AMT  = (SEND_DMA_SIZE-1);                  //字节数
        DMA_UR4T_TXAH = (u16)(&SEND_BUF[SEND_POINT])>>8;    //地址
        DMA_UR4T_TXAL = (u16)(&SEND_BUF[SEND_POINT]);       //地址
...
u16 UART4_Receive(u8 *buf, u16 Size)
{
    u16 w_addr,nu=0;
    if(Size==0)return 0;
    w_addr=DmaUr4rDoneNu();
    if(Receive_R_ADDR==w_addr)return 0;
    nu = (w_addr>Receive_R_ADDR)? w_addr-Receive_R_ADDR : w_addr+UART4_RECEIVE_BUF_SIZE-Receive_R_ADDR;
    nu = (nu<Size)? nu:Size;
    if((Receive_R_ADDR+nu)<UART4_RECEIVE_BUF_SIZE){
        memcpy(buf,&Receive_BUF[Receive_R_ADDR],nu);
        Receive_R_ADDR+=nu;
        return nu;
    }
    else{
        u16 len = UART4_RECEIVE_BUF_SIZE-Receive_R_ADDR;
...
static void DMA_UART4_Init(void)            
{
    ...
    DMA_UR4R_AMTH = (UART4_RECEIVE_BUF_SIZE-1)>>8;	//字节数
    DMA_UR4R_AMT  = (UART4_RECEIVE_BUF_SIZE-1);	    //字节数		
    DMA_UR4R_RXAH = (u16)Receive_BUF>>8;            //地址
    DMA_UR4R_RXAL = (u16)Receive_BUF;               //地址
...
void DMA_UART4TX_ISR_Handler(void) interrupt DMA_UR4T_VECTOR  
{
    ...
    if(SEND_POINT>=UART4_SEND_BUF_SIZE)SEND_POINT=0;//指针循环调整
