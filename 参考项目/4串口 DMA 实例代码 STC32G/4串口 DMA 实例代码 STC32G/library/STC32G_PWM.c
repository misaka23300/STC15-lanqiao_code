...
u8 PWM_Configuration( u8 PWM, PWMx_InitDefine *PWMx )
{
	if ( PWM == PWM1 )
	{
		PWMA_CC1E_Disable();		//ر벶/Ƚ
		PWMA_CC1NE_Disable();		//رձȽ
		PWMA_CC1S_Direction( CCAS_OUTPUT );		//CCnSͨرʱǿд
		PWMA_OC1ModeSet( PWMx->PWM_Mode );		//Ƚģʽ
	...
	if ( PWM == PWM2 )
	{
		PWMA_CC2E_Disable();		//ر벶/Ƚ
		PWMA_CC2NE_Disable();		//رձȽ
		PWMA_CC2S_Direction( CCAS_OUTPUT );		//CCnSͨرʱǿд
		PWMA_OC2ModeSet( PWMx->PWM_Mode );		//Ƚģʽ
	...
	if ( PWM == PWM3 )
	{
		PWMA_CC3E_Disable();		//ر벶/Ƚ
		PWMA_CC3NE_Disable();		//رձȽ
		PWMA_CC3S_Direction( CCAS_OUTPUT );		//CCnSͨرʱǿд
		PWMA_OC3ModeSet( PWMx->PWM_Mode );		//Ƚģʽ
	...
	if ( PWM == PWM4 )
	{
		PWMA_CC4E_Disable();		//ر벶/Ƚ
		PWMA_CC4NE_Disable();		//رձȽ
		PWMA_CC4S_Direction( CCAS_OUTPUT );		//CCnSͨرʱǿд
		PWMA_OC4ModeSet( PWMx->PWM_Mode );		//Ƚģʽ
	...
	if ( PWM == PWMA )
	{
//		PWMA_OC1_ReloadEnable( PWMx->PWM_Reload );	//ȽϵԤװʹ
//		PWMA_OC1_FastEnable( PWMx->PWM_Fast );		//ȽϿٹʹ
//		PWMA_CCPCAPreloaded( PWMx->PWM_PreLoad );	///ȽԤװؿλ(λֻԾлͨ)
//		PWMA_BrakeEnable( PWMx->PWM_BrakeEnable );	///ֹɲ
		...
		PWMA_CEN_Enable( PWMx->PWM_CEN_Enable );	//ʹܼ
		return	SUCCESS;
	}

	if ( PWM == PWM5 )
	{
		PWMB_CC5E_Disable();		//ر벶/Ƚ
		PWMB_CC5S_Direction( CCAS_OUTPUT );		//CCnSͨرʱǿд
		PWMB_OC5ModeSet( PWMx->PWM_Mode );		//Ƚģʽ
	...
	if ( PWM == PWM6 )
	{
		PWMB_CC6E_Disable();		//ر벶/Ƚ
		PWMB_CC6S_Direction( CCAS_OUTPUT );		//CCnSͨرʱǿд
		PWMB_OC6ModeSet( PWMx->PWM_Mode );		//Ƚģʽ
	...
	if ( PWM == PWM7 )
	{
		PWMB_CC7E_Disable();		//ر벶/Ƚ
		PWMB_CC7S_Direction( CCAS_OUTPUT );		//CCnSͨرʱǿд
		PWMB_OC7ModeSet( PWMx->PWM_Mode );		//Ƚģʽ
	...
	if ( PWM == PWM8 )
	{
		PWMB_CC8E_Disable();		//ر벶/Ƚ
		PWMB_CC8S_Direction( CCAS_OUTPUT );		//CCnSͨرʱǿд
		PWMB_OC8ModeSet( PWMx->PWM_Mode );		//Ƚģʽ
	...
	if ( PWM == PWMB )
	{
//		PWMB_OC5_ReloadEnable( PWMx->PWM_Reload );	//ȽϵԤװʹ
//		PWMB_OC5_FastEnable( PWMx->PWM_Fast );		//ȽϿٹʹ
//		PWMB_CCPCBPreloaded( PWMx->PWM_PreLoad );	///ȽԤװؿλ(λֻԾлͨ)
//		PWMB_BrakeEnable( PWMx->PWM_BrakeEnable );	///ֹɲ
		...
		PWMB_CEN_Enable( PWMx->PWM_CEN_Enable );	//ʹܼ
		return	SUCCESS;
	}
...
u8 ReadPWMA( u8 addr )
{
	...
	while ( HSPWMA_ADR &	0x80 ); //ȴǰһ첽д
	HSPWMA_ADR = addr |	0x80;  //üӷʵַ,ֻҪԭXFRַĵ7λ
								//HSPWMA_ADDRĴλд1,ʾ
	while ( HSPWMA_ADR &	0x80 ); //ȴǰ첽ȡ
	dat	= HSPWMA_DAT;		   //ȡ첽
...
u8 ReadPWMB( u8 addr )
{
	...
	while ( HSPWMB_ADR &	0x80 ); //ȴǰһ첽д
	HSPWMB_ADR = addr |	0x80;  //üӷʵַ,ֻҪԭXFRַĵ7λ
								//HSPWMB_ADDRĴλд1,ʾ
	while ( HSPWMB_ADR &	0x80 ); //ȴǰ첽ȡ
	dat	= HSPWMB_DAT;		   //ȡ첽
...
void WritePWMA( u8	addr, u8 dat )
{
	while ( HSPWMA_ADR &	0x80 ); //ȴǰһ첽д
	HSPWMA_DAT = dat;		   //׼Ҫд
	HSPWMA_ADR = addr &	0x7f;  //üӷʵַ,ֻҪԭXFRַĵ7λ
								//HSPWMA_ADDRĴλд0,ʾд
}
...
void WritePWMB( u8	addr, u8 dat )
{
	while ( HSPWMB_ADR &	0x80 ); //ȴǰһ첽д
	HSPWMB_DAT = dat;		   //׼Ҫд
	HSPWMB_ADR = addr &	0x7f;  //üӷʵַ,ֻҪԭXFRַĵ7λ
								//HSPWMB_ADDRĴλд0,ʾд
}
...
void HSPWM_Configuration( u8 PWM, HSPWMx_InitDefine *PWMx, PWMx_Duty *DUTYx )
{
	...
	if ( PWM == PWMA )
	{
		...
		WritePWMA(( u8 )&PWMA_CCMR1, CCMRn_PWM_MODE1 );			//ͨģʽ
		WritePWMA(( u8 )&PWMA_CCMR2, CCMRn_PWM_MODE1 );
		WritePWMA(( u8 )&PWMA_CCMR3, CCMRn_PWM_MODE1 );
		WritePWMA(( u8 )&PWMA_CCMR4, CCMRn_PWM_MODE1 );
		WritePWMA(( u8 )&PWMA_CCER1, tmpCCER1 );			//ͨʹܺͼ
		WritePWMA(( u8 )&PWMA_CCER2, tmpCCER2 );
		WritePWMA(( u8 )&PWMA_ENO, tmpENO );	//ʹPWMź˿
		WritePWMA(( u8 )&PWMA_CCR1H, ( u8 )( DUTYx->PWM1_Duty >> 8 ));	//PWMռձ
		WritePWMA(( u8 )&PWMA_CCR1L, ( u8 )DUTYx->PWM1_Duty );
		WritePWMA(( u8 )&PWMA_CCR2H, ( u8 )( DUTYx->PWM2_Duty >> 8 ));	//PWMռձ
		WritePWMA(( u8 )&PWMA_CCR2L, ( u8 )DUTYx->PWM2_Duty );
		WritePWMA(( u8 )&PWMA_CCR3H, ( u8 )( DUTYx->PWM3_Duty >> 8 ));	//PWMռձ
		WritePWMA(( u8 )&PWMA_CCR3L, ( u8 )DUTYx->PWM3_Duty );
		WritePWMA(( u8 )&PWMA_CCR4H, ( u8 )( DUTYx->PWM4_Duty >> 8 ));	//PWMռձ
		...
		WritePWMA(( u8 )&PWMA_DTR, PWMx->PWM_DeadTime );					//ûԳPWM
		WritePWMA(( u8 )&PWMA_BKR, PWMx->PWM_MainOutEnable<<7 );	//ʹ
		WritePWMA(( u8 )&PWMA_CR1, PWMx->PWM_CEN_Enable );				//ʼPWM
	}
	else if ( PWM == PWMB )
	{
		HSPWMB_CFG = 0x03;			//ʹPWMBؼĴ첽ʹ
		...
		WritePWMB(( u8 )&PWMB_CCMR1, CCMRn_PWM_MODE1 );			//ͨģʽ
		WritePWMB(( u8 )&PWMB_CCMR2, CCMRn_PWM_MODE1 );
		WritePWMB(( u8 )&PWMB_CCMR3, CCMRn_PWM_MODE1 );
		WritePWMB(( u8 )&PWMB_CCMR4, CCMRn_PWM_MODE1 );
		WritePWMB(( u8 )&PWMB_CCER1, tmpCCER1 );			//ͨʹܺͼ
		WritePWMB(( u8 )&PWMB_CCER2, tmpCCER2 );
		WritePWMB(( u8 )&PWMB_ENO, tmpENO );	//ʹPWMź˿
		WritePWMB(( u8 )&PWMB_CCR5H, ( u8 )( DUTYx->PWM5_Duty >> 8 ));	//PWMռձ
		WritePWMB(( u8 )&PWMB_CCR5L, ( u8 )DUTYx->PWM5_Duty );
		WritePWMB(( u8 )&PWMB_CCR6H, ( u8 )( DUTYx->PWM6_Duty >> 8 ));	//PWMռձ
		WritePWMB(( u8 )&PWMB_CCR6L, ( u8 )DUTYx->PWM6_Duty );
		WritePWMB(( u8 )&PWMB_CCR7H, ( u8 )( DUTYx->PWM7_Duty >> 8 ));	//PWMռձ
		WritePWMB(( u8 )&PWMB_CCR7L, ( u8 )DUTYx->PWM7_Duty );
		WritePWMB(( u8 )&PWMB_CCR8H, ( u8 )( DUTYx->PWM8_Duty >> 8 ));	//PWMռձ
		...
		WritePWMB(( u8 )&PWMB_DTR, PWMx->PWM_DeadTime );					//ûԳPWM
		WritePWMB(( u8 )&PWMB_BKR, PWMx->PWM_MainOutEnable<<7 );	//ʹ
...
void UpdateHSPwm( u8 PWM, PWMx_Duty *PWMx )
{
	...
	{
		case PWM1:
			WritePWMA(( u8 )&PWMA_CCR1H, ( u8 )( PWMx->PWM1_Duty >> 8 ));	//PWMռձ
			WritePWMA(( u8 )&PWMA_CCR1L, ( u8 )PWMx->PWM1_Duty );
		break;

		case PWM2:
			WritePWMA(( u8 )&PWMA_CCR2H, ( u8 )( PWMx->PWM2_Duty >> 8 ));	//PWMռձ
			WritePWMA(( u8 )&PWMA_CCR2L, ( u8 )PWMx->PWM2_Duty );
		break;

		case PWM3:
			WritePWMA(( u8 )&PWMA_CCR3H, ( u8 )( PWMx->PWM3_Duty >> 8 ));	//PWMռձ
			WritePWMA(( u8 )&PWMA_CCR3L, ( u8 )PWMx->PWM3_Duty );
		break;

		case PWM4:
			WritePWMA(( u8 )&PWMA_CCR4H, ( u8 )( PWMx->PWM4_Duty >> 8 ));	//PWMռձ
			WritePWMA(( u8 )&PWMA_CCR4L, ( u8 )PWMx->PWM4_Duty );
		break;

		case PWM5:
			WritePWMB(( u8 )&PWMB_CCR5H, ( u8 )( PWMx->PWM5_Duty >> 8 ));	//PWMռձ
			WritePWMB(( u8 )&PWMB_CCR5L, ( u8 )PWMx->PWM5_Duty );
		break;

		case PWM6:
			WritePWMB(( u8 )&PWMB_CCR6H, ( u8 )( PWMx->PWM6_Duty >> 8 ));	//PWMռձ
			WritePWMB(( u8 )&PWMB_CCR6L, ( u8 )PWMx->PWM6_Duty );
		break;

		case PWM7:
			WritePWMB(( u8 )&PWMB_CCR7H, ( u8 )( PWMx->PWM7_Duty >> 8 ));	//PWMռձ
			WritePWMB(( u8 )&PWMB_CCR7L, ( u8 )PWMx->PWM7_Duty );
		break;

		case PWM8:
			WritePWMB(( u8 )&PWMB_CCR8H, ( u8 )( PWMx->PWM8_Duty >> 8 ));	//PWMռձ
