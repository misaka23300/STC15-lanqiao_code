...
#define		SPI_SSIG_Set( n )					SSIG = n		/* SSŹܿ */
#define		SPI_Start( n )						SPEN = n		/* SPIʹܿλ */
#define		SPI_FirstBit_Set( n )			DORD = n		/* ݷ/˳ MSB / SB */
#define		SPI_Mode_Set( n )					MSTR = n		/* SPIģʽ */
#define		SPI_CPOL_Set( n )					CPOL = n		/* SPIʱӼԿ */
#define		SPI_CPHA_Set( n )					CPHA = n		/* SPIʱλ */
#define		SPI_Clock_Select( n )			SPCTL = ( SPCTL & ~0x03 ) | ( n )					/* SPIʱƵѡ */

#define		SPI_ClearFlag()					{SPIF = 1; WCOL = 1;}		/* д 1  SPIFWCOL ־ */

#define		HSSPI_Enable( n )					HSSPI_CFG2 |= 0x20       //ʹSPIģʽ
#define		HSSPI_Disable( n )				HSSPI_CFG2 &= ~0x20      //رSPIģʽ
...
typedef struct
{
	...
	u8	SPI_SSIG;			//Ƭѡλ, ENABLE( SSȷӻ), DISABLE( SSŹ)
