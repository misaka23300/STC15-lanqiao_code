...
发送部分定义及变量
----------------------------------------------------------*/
static u8 xdata  SEND_BUF[UART1_SEND_BUF_SIZE]; //数据池
...
接收部分定义及变量
----------------------------------------------------------*/
static u8 xdata  Receive_BUF[UART1_RECEIVE_BUF_SIZE];  //数据池
...
{    
    ...
    if((end+Size)<UART1_SEND_BUF_SIZE){      //复制数据
        memcpy(&SEND_BUF[end],buf,Size);
        end+=Size;
    }
    else{
        u16 len = UART1_SEND_BUF_SIZE-end;
...
u16 UART1_Receive(u8 *buf, u16 Size)
{
    ...
    nu = (w_addr>Receive_R_ADDR)? w_addr-Receive_R_ADDR : w_addr+UART1_RECEIVE_BUF_SIZE-Receive_R_ADDR;
    nu = (nu<Size)? nu:Size;
    if((Receive_R_ADDR+nu)<UART1_RECEIVE_BUF_SIZE){
        memcpy(buf,&Receive_BUF[Receive_R_ADDR],nu);
        Receive_R_ADDR+=nu;
        return nu;
    }
    else{
        u16 len = UART1_RECEIVE_BUF_SIZE-Receive_R_ADDR;
...
static void DMA_UART1_Init(void)            
{
    ...
    DMA_UR1R_AMTH = (UART1_RECEIVE_BUF_SIZE-1)>>8;	//字节数
    DMA_UR1R_AMT  = (UART1_RECEIVE_BUF_SIZE-1);	    //字节数		
    DMA_UR1R_RXAH = (u16)Receive_BUF>>8;            //地址
    DMA_UR1R_RXAL = (u16)Receive_BUF;               //地址
