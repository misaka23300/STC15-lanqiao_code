...
void	CMP_Inilize( CMP_InitDefine *CMPx )
{
	...
	if ( CMPx->CMP_Outpt_En == ENABLE )		CMPOE = 1;			//允许比较结果输出到P3.4 / 4.1,   ENABLE, DISABLE
	if ( CMPx->CMP_InvCMPO     == ENABLE )		INVCMPO = 1;	//比较器输出取反, 	ENABLE, DISABLE
	if ( CMPx->CMP_100nsFilter == DISABLE )	DISFLT = 1;		//内部0.1uF滤波,  	ENABLE, DISABLE

	if ( CMPx->CMP_P_Select  <= CMP_P_ADC )	CMPEXCFG = ( CMPEXCFG & ~0x03 ) | ( CMPx->CMP_P_Select );			//比较器输入正极选择, CMP_P_P37 / MP_P_P50 / MP_P_P51, CMP_P_ADC: 由ADC模拟输入端做正输入.
