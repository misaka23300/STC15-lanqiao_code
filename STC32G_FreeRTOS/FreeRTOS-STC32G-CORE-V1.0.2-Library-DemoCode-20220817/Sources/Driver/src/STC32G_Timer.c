...
//========================================================================
// 函数: uint8_t	Timer_Inilize( uint8_t TIM, TIM_InitTypeDef *TIMx )
// 描述: 定时器初始化程序.
...
uint8_t	Timer_Inilize( uint8_t TIM, TIM_InitTypeDef *TIMx )
{
	if ( TIM == Timer0 )
	{
		...
		Timer0_CLK_Select( TIMx->TIM_ClkSource );	//对外计数或分频, 定时12T / T
	...
	if ( TIM == Timer1 )
	{
		...
		if ( TIMx->TIM_ClkSource >  TIM_CLOCK_Ext )	return FAIL;
		Timer1_CLK_Select( TIMx->TIM_ClkSource );	//对外计数或分频, 定时12T / T
	...
	{
		Timer2_Stop();	//停止计数
		Timer2_CLK_Select( TIMx->TIM_ClkSource );	//对外计数或分频, 定时12T / T
	...
	if ( TIM == Timer3 )		//Timer3,固定为16位自动重装, 中断无优先级
	{
		Timer3_Stop();	//停止计数
		if ( TIMx->TIM_ClkSource >  TIM_CLOCK_Ext )	return FAIL;
		Timer3_CLK_Select( TIMx->TIM_ClkSource );	//对外计数或分频, 定时12T / T
	...
	if ( TIM == Timer4 )		//Timer3,固定为16位自动重装, 中断无优先级
	{
		Timer4_Stop();	//停止计数
		if ( TIMx->TIM_ClkSource >  TIM_CLOCK_Ext )	return FAIL;
		Timer4_CLK_Select( TIMx->TIM_ClkSource );	//对外计数或分频, 定时12T / T
