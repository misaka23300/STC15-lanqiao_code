...
uint8_t UART_Configuration( uint8_t UARTx, COMx_InitDefine *COMx )
{
	...
	if ( UARTx == UART1 )
	{
		...
		{
			if ( COMx->BaudRateDouble == ENABLE )	S1M0x6 = 1;	//固定波特率SysClk / 
			else								S1M0x6 = 0;	//固定波特率SysClk / 2
		}
		else if ( COMx->UART_Mode == UART_9bit )	//固定波特率SysClk * ^SMOD / 4
		{
			if ( COMx->BaudRateDouble == ENABLE )	SMOD = 1;	//固定波特率SysClk / 2
			else								SMOD = 0;	//固定波特率SysClk / 4
		}
