...
#define		PWM2_USE_P12P13()		PWMA_PS = ( PWMA_PS & ~0x0C )					/* PWM 通道 2 输出脚切换到P12 / 54( PWM2P ) P13( PWM2N ) */ 
...
#define PWMA_OPMA( n )							( n == ?( PWMA_CR1 |= 0x08 ):( PWMA_CR1 &= ~0x08 ))	//单脉冲模式 0：在发生更新事件时，计数器不停止；1：在发生下一次更新事件时，清除 CEN 位，计数器停止
#define PWMA_URSA( n )							( n == ?( PWMA_CR1 |= 0x04 ):( PWMA_CR1 &= ~0x04 ))	//更新请求源
#define PWMA_UDISA( n )							( n == ?( PWMA_CR1 |= 0x02 ):( PWMA_CR1 &= ~0x02 ))	//禁止更新 0：产生更新（UEV）事件；1：不产生更新事件
//#define PWMA_CEN_Enable()					PWMA_CR1 |= 0x01		//1：使能计数器
#define PWMA_CEN_Disable()				PWMA_CR1 &= ~0x01		//0：禁止计数器
#define PWMA_CEN_Enable( n )				( n == ?( PWMA_CR1 |= 0x01 ):( PWMA_CR1 &= ~0x01 ))		//1：使能计数器
...
#define PWMB_OPMB( n )							( n == ?( PWMB_CR1 |= 0x08 ):( PWMB_CR1 &= ~0x08 ))	//单脉冲模式 0：在发生更新事件时，计数器不停止；1：在发生下一次更新事件时，清除 CEN 位，计数器停止
#define PWMB_URSB( n )							( n == ?( PWMB_CR1 |= 0x04 ):( PWMB_CR1 &= ~0x04 ))	//更新请求源
#define PWMB_UDISB( n )							( n == ?( PWMB_CR1 |= 0x02 ):( PWMB_CR1 &= ~0x02 ))	//禁止更新 0：产生更新（UEV）事件；1：不产生更新事件
//#define PWMB_CEN_Enable()					PWMB_CR1 |= 0x01		//1：使能计数器
#define PWMB_CEN_Disable()				PWMB_CR1 &= ~0x01		//0：禁止计数器
#define PWMB_CEN_Enable( n )				( n == ?( PWMB_CR1 |= 0x01 ):( PWMB_CR1 &= ~0x01 ))		//1：使能计数器
...
//0：当 CCPC =  时，只有在 COMG 位置 1 的时候这些控制位才被更新
//1：当 CCPC =  时，只有在 COMG 位置 1 或 TRGI 发生上升沿的时候这些控制位才被更新
#define PWMA_COMSUpdateCtrl( n )		PWMA_CR2 = ( n == ?( PWMA_CR2 |= 0x04 ):( PWMA_CR2 &= ~0x04 ))	//捕获/比较控制位的更新控制选择
#define PWMB_COMSUpdateCtrl( n )		PWMB_CR2 = ( n == ?( PWMB_CR2 |= 0x04 ):( PWMB_CR2 &= ~0x04 ))	//捕获/比较控制位的更新控制选择
//0： CCIE， CCINE， CCiP， CCiNP 和 OCIM 位不是预装载的
//1： CCIE， CCINE， CCiP， CCiNP 和 OCIM 位是预装载的；设置该位后，它们只在设置了 COMG位后被更新。
#define PWMA_CCPCAPreloaded( n )		PWMA_CR2 = ( n == ?( PWMA_CR2 |= 0x01 ):( PWMA_CR2 &= ~0x01 ))	//捕获/比较预装载控制位(该位只对具有互补输出的通道起作用)
#define PWMB_CCPCBPreloaded( n )		PWMA_CR2 = ( n == ?( PWMA_CR2 |= 0x01 ):( PWMA_CR2 &= ~0x01 ))	//捕获/比较预装载控制位(该位只对具有互补输出的通道起作用)
...
#define PWMA_OC1_ReloadEnable( n )	( n == ?( PWMA_CCMR1 |= 0x08 ):( PWMA_CCMR1 &= ~0x08 ))		//开启 OC1PE 输出比较的预装载功能
#define PWMB_OC5_ReloadEnable( n )	( n == ?( PWMB_CCMR1 |= 0x08 ):( PWMB_CCMR1 &= ~0x08 ))		//开启 OC5PE 输出比较的预装载功能
...
#define PWMA_OC1_FastEnable( n )		( n == ?( PWMA_CCMR1 |= 0x04 ):( PWMA_CCMR1 &= ~0x04 ))		//1：开启 OC1FE 输出比较快速功能
#define PWMB_OC5_FastEnable( n )		( n == ?( PWMB_CCMR1 |= 0x04 ):( PWMB_CCMR1 &= ~0x04 ))		//1：开启 OC5FE 输出比较快速功能
...
#define CCAS_IUTPUT_TI1FP1		0x01		//输入，IC1 / C2 / C3 / C4 映射在 TI1FP1 上
#define CCAS_IUTPUT_TI2FP1		0x02		//输入，IC1 / C2 / C3 / C4 映射在 TI2FP1 上
#define CCAS_IUTPUT_TRC				0x03		//输入，IC1 / C2 / C3 / C4 映射在 TRC 上
#define CCBS_OUTPUT						0x00		//输出
#define CCBS_IUTPUT_TI5FP5		0x01		//输入，IC5 / C6 / C7 / C8 映射在 TI5FP5 上
#define CCBS_IUTPUT_TI6FP5		0x02		//输入，IC5 / C6 / C7 / C8 映射在 TI6FP5 上
#define CCBS_IUTPUT_TRC				0x03		//输入，IC5 / C6 / C7 / C8 映射在 TRC 上
...
#define PWMA_BrakeOutputEnable( n )				( n == ?( PWMA_BRK |= 0x80 ):( PWMA_BRK &= ~0x80 ))		//1：主输出使能
#define PWMB_BrakeOutputEnable( n )				( n == ?( PWMB_BRK |= 0x80 ):( PWMB_BRK &= ~0x80 ))		//1：主输出使能
...
#define PWMA_BrakeEnable( n )							( n == ?( PWMA_BRK |= 0x10 ):( PWMA_BRK &= ~0x10 ))		//1：开启刹车输入
#define PWMB_BrakeEnable( n )							( n == ?( PWMB_BRK |= 0x10 ):( PWMB_BRK &= ~0x10 ))		//1：开启刹车输入
...
//运行模式下“关闭状态”选择
#define PWMA_OSSRnEnable()							PWMA_BRK |= 0x08		//1：当 PWM 不工作时，一旦 CCiE =  或 CCiNE = ，首先开启 OC / CN 并输出无效电平，然后置OC / CN 使能输出信号=1
#define PWMA_OSSRnDisable()							PWMA_BRK &= ~0x08		//0：当 PWM 不工作时，禁止 OC / CN 输出（OC / CN 使能输出信号=0）
#define PWMB_OSSRnEnable()							PWMB_BRK |= 0x08		//1：当 PWM 不工作时，一旦 CCiE =  或 CCiNE = ，首先开启 OC / CN 并输出无效电平，然后置OC / CN 使能输出信号=1
#define PWMB_OSSRnDisable()							PWMB_BRK &= ~0x08		//0：当 PWM 不工作时，禁止 OC / CN 输出（OC / CN 使能输出信号=0）
//空闲模式下“关闭状态”选择
#define PWMA_OSSInEnable()							PWMA_BRK |= 0x04		//1：当 PWM 不工作时，一旦 CCiE =  或 CCiNE = ，OC / CN 首先输出其空闲电平，然后 OC / CN使能输出信号=1
#define PWMA_OSSInDisable()							PWMA_BRK &= ~0x04		//0：当 PWM 不工作时，禁止 OC / CN 输出（OC / CN 使能输出信号=0）
#define PWMB_OSSInEnable()							PWMB_BRK |= 0x04		//1：当 PWM 不工作时，一旦 CCiE =  或 CCiNE = ，OC / CN 首先输出其空闲电平，然后 OC / CN使能输出信号=1
#define PWMB_OSSInDisable()							PWMB_BRK &= ~0x04		//0：当 PWM 不工作时，禁止 OC / CN 输出（OC / CN 使能输出信号=0）

#define PWMn_lock_L0			0		//寄存器无写保护
#define PWMn_lock_L1			1		//锁定级别 1：不能写入 PWMn_BKR 寄存器的 BKE、BKP、AOE 位和PWMn_OISR 寄存器的 OISI 位
#define PWMn_lock_L2			2		//锁定级别 2：不能写入锁定级别 1 中的各位，也不能写入 CC 极性位以及 OSSR / SSI 位
...
#define PWMA_OC1_OUT_0()					PWMA_OISR &= ~0x01	/* 当 MOE =  时，如果 OC1N 使能，则在一个死区后，OC1 =  */
#define PWMA_OC1_OUT_1()					PWMA_OISR |= 0x01		/* 当 MOE =  时，如果 OC1N 使能，则在一个死区后，OC1 =  */
#define PWMA_OC1N_OUT_0()					PWMA_OISR &= ~0x02	/* 当 MOE =  时，则在一个死区后，OC1N =  */
#define PWMA_OC1N_OUT_1()					PWMA_OISR |= 0x02		/* 当 MOE =  时，则在一个死区后，OC1N =  */
#define PWMA_OC2_OUT_0()					PWMA_OISR &= ~0x04	/* 当 MOE =  时，如果 OC2N 使能，则在一个死区后，OC2 =  */
#define PWMA_OC2_OUT_1()					PWMA_OISR |= 0x04		/* 当 MOE =  时，如果 OC2N 使能，则在一个死区后，OC2 =  */
#define PWMA_OC2N_OUT_0()					PWMA_OISR &= ~0x08	/* 当 MOE =  时，则在一个死区后，OC2N =  */
#define PWMA_OC2N_OUT_1()					PWMA_OISR |= 0x08		/* 当 MOE =  时，则在一个死区后，OC2N =  */
#define PWMA_OC3_OUT_0()					PWMA_OISR &= ~0x10	/* 当 MOE =  时，如果 OC3N 使能，则在一个死区后，OC3 =  */
#define PWMA_OC3_OUT_1()					PWMA_OISR |= 0x10		/* 当 MOE =  时，如果 OC3N 使能，则在一个死区后，OC3 =  */
#define PWMA_OC3N_OUT_0()					PWMA_OISR &= ~0x20	/* 当 MOE =  时，则在一个死区后，OC3N =  */
#define PWMA_OC3N_OUT_1()					PWMA_OISR |= 0x20		/* 当 MOE =  时，则在一个死区后，OC3N =  */
#define PWMA_OC4_OUT_0()					PWMA_OISR &= ~0x40	/* 当 MOE =  时，如果 OC4N 使能，则在一个死区后，OC4 =  */
#define PWMA_OC4_OUT_1()					PWMA_OISR |= 0x40		/* 当 MOE =  时，如果 OC4N 使能，则在一个死区后，OC4 =  */
#define PWMA_OC4N_OUT_0()					PWMA_OISR &= ~0x80	/* 当 MOE =  时，则在一个死区后，OC4N =  */
#define PWMA_OC4N_OUT_1()					PWMA_OISR |= 0x80		/* 当 MOE =  时，则在一个死区后，OC4N =  */

#define PWMB_OC5_OUT_0()					PWMB_OISR &= ~0x01	/* 当 MOE =  时，则在一个死区后，OC5 =  */
#define PWMB_OC5_OUT_1()					PWMB_OISR |= 0x01		/* 当 MOE =  时，则在一个死区后，OC5 =  */
#define PWMB_OC6_OUT_0()					PWMB_OISR &= ~0x04	/* 当 MOE =  时，则在一个死区后，OC6 =  */
#define PWMB_OC6_OUT_1()					PWMB_OISR |= 0x04		/* 当 MOE =  时，则在一个死区后，OC6 =  */
#define PWMB_OC7_OUT_0()					PWMB_OISR &= ~0x10	/* 当 MOE =  时，则在一个死区后，OC7 =  */
#define PWMB_OC7_OUT_1()					PWMB_OISR |= 0x10		/* 当 MOE =  时，则在一个死区后，OC7 =  */
#define PWMB_OC8_OUT_0()					PWMB_OISR &= ~0x40	/* 当 MOE =  时，则在一个死区后，OC8 =  */
#define PWMB_OC8_OUT_1()					PWMB_OISR |= 0x40		/* 当 MOE =  时，则在一个死区后，OC8 =  */
...
typedef struct
{ 
	uint8_t	PWM_Mode;			//模式,   CCMRn_FREEZE, CCMRn_MATCH_VALID, CCMRn_MATCH_INVALID, CCMRn_ROLLOVER, CCMRn_FORCE_INVALID, CCMRn_FORCE_VALID, CCMRn_PWM_MODE1, CCMRn_PWM_MODE2
	uint16_t	PWM_Period;		//周期时间,   0~65535
	uint16_t	PWM_Duty;			//占空比时间, 0~Period
	uint8_t	PWM_DeadTime;	//死区发生器设置, 0~255
//	uint8_t	PWM_Reload;				//输出比较的预装载使能,   ENABLE, DISABLE
//	uint8_t	PWM_Fast;					//输出比较快速功能使能,   ENABLE, DISABLE
//	uint8_t	PWM_PreLoad;			//预装载,     ENABLE, DISABLE
//	uint8_t	PWM_BrakeEnable;	//刹车输入使能,  ENABLE, DISABLE
	uint8_t	PWM_EnoSelect;		//输出通道选择,	ENO1P, ENO1N, ENO2P, ENO2N, ENO3P, ENO3N, ENO4P, ENO4N / ENO5P, ENO6P, ENO7P, ENO8P
	uint8_t	PWM_CEN_Enable;		//使能计数器, ENABLE, DISABLE
	uint8_t	PWM_MainOutEnable;//主输出使能,  ENABLE, DISABLE
} PWMx_InitDefine; 

typedef struct
{ 
	uint16_t	PWM1_Duty;			//PWM1占空比时间, 0~Period
	uint16_t	PWM2_Duty;			//PWM2占空比时间, 0~Period
	uint16_t	PWM3_Duty;			//PWM3占空比时间, 0~Period
	uint16_t	PWM4_Duty;			//PWM4占空比时间, 0~Period
	uint16_t	PWM5_Duty;			//PWM5占空比时间, 0~Period
	uint16_t	PWM6_Duty;			//PWM6占空比时间, 0~Period
	uint16_t	PWM7_Duty;			//PWM7占空比时间, 0~Period
	uint16_t	PWM8_Duty;			//PWM8占空比时间, 0~Period
} PWMx_Duty; 
