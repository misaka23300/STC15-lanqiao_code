...
#define		I2C_Function( n )	( n == ?( I2CCFG &= ~0x80 ):( I2CCFG |= 0x80 ))	//0：禁止 I2C 功能；1：使能 I2C 功能
...
#define		I2C_SetSpeed( n )	I2CCFG = ( I2CCFG & ~0x3f ) | ( n & 0x3f )	/* 总线速度=Fosc / /( Speed *  + ) */
...
typedef struct
{
	uint8_t	I2C_Speed;				//总线速度=Fosc / /( Speed *  + ),      0~63
...
extern uint8_t I2C_BUF_type I2C_Buffer[I2C_BUF_LENTH];
extern bit I2C_DisplayFlag;

void	I2C_Init( I2C_InitTypeDef *I2Cx );
void	I2C_WriteNbyte( uint8_t addr, uint8_t *p, uint8_t number );
void	I2C_ReadNbyte( uint8_t addr, uint8_t *p, uint8_t number );
