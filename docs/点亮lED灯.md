...
**74HC573** 是一种 **8 位锁存器（Latch）**，属于 **74HC 系列高速 CMOS 逻辑器件**。
 它的主要功能是 **暂存数据** 并 **控制输出**，常用于 **数据缓冲、总线驱动、地址锁存** 等场合。
...
| 功能                           | 描述                                                         |
| ------------------------------ | ------------------------------------------------------------ |
| **锁存数据（Latch）**          | 当锁存控制信号（LE）为高电平时，输入端的数据可直接透传到输出端。<br>当 LE 变为低电平时，输出端保持（锁存）最后一个输入状态。 |
| **三态输出（3-State Output）** | 通过 OE 控制输出是否连接到总线。OE = 1 时输出高阻态（Hi-Z），OE = 0 时输出有效数据。 |
...
| ---- | ---- | -------------------------------------- |
| H    | L    | **透明状态**：Q 输出跟随 D输入         |
| L    | L    | **锁存状态**：Q输出 保持最后一次输入值 |
| X    | H    | **高阻态**：输出断开，不影响总线       |

这颗芯片是锁存器，主要是锁存的功能。锁存，就是输入一串数据，LE引脚为高电平时芯片处于透明模式，输出这个数据，使用控制引脚将输出的数据锁住，无论输入数据如何变化也不影响输出数据。

LE引脚是模式切换引脚，LE引脚为高电平时，芯片处于透明模式，8个输入引脚会呈现在输出引脚上。LE引脚为低电平时，芯片处于锁存状态。输出端Q的电平为LE引脚下降沿之前的输入端的引脚。

也就是说，我们可以先配置输入端的电平，然后LE引脚由低电平切换成高电平(上升沿)，然后切换低电平(下降沿)。在这个下降沿，

OE引脚为使能引脚，低电平有效。如果OE引脚为高电平，则输出引脚为高阻态关闭状态，OE引脚为低电平时，LE可以控制芯片的透明/锁存状态。
...
1. **地址锁存**（常见于 8088、8051 总线分离）
   当 LE 信号为高电平时，地址线信号透传；LE 下降延后锁存地址。
2. **I/O 缓冲**
   保护 CPU 数据总线，避免外设直接干扰。
3. **数据暂存**
   在不同模块之间传递数据时作为“数据中转站”。
...
| -------------- | ------ | ------- |
| 工作电压       | VCC    | 2V ~ 6V |
| 输入高电平阈值 | VCC=5V | 3.15V   |
| 输入低电平阈值 | VCC=5V | 1.35V   |
| 输出电流       | VCC=5V | ±6mA    |
| 最大工作频率   | VCC=5V | ~30MHz  |
