...
void taskControl( void )
{
    ...
    if ( f_col == 1 ){
        ...
        if (( frq < 200 )||( frq > 2000 )){
            ...
            /* hum = 2frq / 5 + 10 /  */
            hum = ( unsigned int )((2 * rq + 50 )/45.0 );
            count++;
        }
        
        if ( hum != 'S'){
            hour = ( now_time[2]/10 * 0 ) + ( now_time[2]%10 );
            min  = ( now_time[1]/10 * 0 ) + ( now_time[1]%10 );
            /* 温度最大值 */
            if ( temp > max_temp ){
                max_temp = temp;
            }
            /* 温度平均值 */
            sum_temp = ( count - )*ave_temp + temp;
            ave_temp = sum_temp / ount;
            ...
            if ( count != 0 ){
                ...
                switch( ui_ec ){
                    ...
                    case 0:
                        set_seg(0, 12 );
                        set_seg(1, 16 );
                        set_seg(2, max_temp / 00 );
                        set_seg(3, max_temp % 00 / 0 );
                        set_seg(4, 17 );
                        set_seg(5, ave_temp / 00 );
                        set_seg(6, ave_temp % 00 / 0 );
                        set_seg(7, ave_temp % 0 );
                        break;
                    /* 湿度回显 */
                    case 1:
                        set_seg(0, 18 );
                        ...
                        set_seg(3, max_hum % 0 );
                        set_seg(4, 17 );
                        set_seg(5, ave_hum / 00 );
                        set_seg(6, ave_hum % 00 / 0 );
                        set_seg(7, ave_hum % 0 );
                        break;
                    /* 时间回显 */
                    case 2:
