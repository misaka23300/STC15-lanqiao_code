...
{
    ISP_CONTR = 0;    // 禁止ISP / AP操作
    ISP_CMD   = 0;    // 去除ISP / AP命令
    ISP_TRIG  = 0;    // 防止ISP / AP命令误触发
...
{
    EA        = 0;                             // 禁止中断
    ISP_CONTR = (ISP_EN + ISP_WAIT_FREQUENCY); // 设置等待时间，允许ISP / AP操作，送一次就够
    ISP_READ();                                // 送字节读命令，命令不需改变时，不需重新送命令
    do {
        ISP_ADDRH = EE_address / 256; // 送地址高字节（地址需要改变时才需重新送地址）
        ISP_ADDRL = EE_address % 256; // 送地址低字节
        ISP_TRIG();                   // 先送5AH，再送A5H到ISP / AP触发寄存器，每次都需要如此
                    // 送完A5H后，ISP / AP命令立即被触发启动
